package app_service_SARS_Calendar;
...
public class App_Service_SARS_Calendar implements App_Service, App_Service_Dependent
{
	private static String service_name = "Календарь";
	...
	private JMenu menu;
	public App_Service_SARS_Calendar()
	{
	...
	//			 								new Class[]{Integer.class,String.class,String.class,String.class});
	  ...
	  // Updater = new MSS_RQ_CxListFiller(TDesc,T_XML_Desc,(Class<? extends ListElementData>) NewsServiceElementData.class);
		...
		Header_lbl.setText("Рабочая область сервиса Календарь.");	
	...
	public void requireInitialization(Object[] params) {
		if (params[0] instanceof SocioProjectMain)
		{
			...
			//JLabel lbl = new JLabel("HERE CALENDAR");
			menu = new JMenu("Календарь");
			JMenuItem watch = new JMenuItem("Просмотр");
			JMenuItem addTask = new JMenuItem("Добавить событие");
			menu.add(watch);
			menu.add(addTask);
			
			MainWindow.getJMenuBar().add(menu);
		
			WorkPane.add(new Calendar_Widget());
		}
	}	
}
